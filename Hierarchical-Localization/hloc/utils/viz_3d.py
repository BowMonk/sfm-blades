...
def plot_reconstruction(
    ...
    if points_rgb:
        ...
        # print(pcolor)
        # pcolor = [[255,255,255] for p3D in p3Ds]
    else:
        pcolor = color
    if points:
...
### ADDED FOR NOISE MEASUREMENT FOR SPARSE RECONSTRUCTION ###
def evaluation_metric(
    ...
    xyzs = [p3D.xyz for p3D in p3Ds]
    xyzs_sorted = np.array(sorted(xyzs, key=lambda cond: cond[2]))
    median = np.median(xyzs_sorted, axis=0)
    pnt_dists_from_median = np.abs(xyzs_sorted - median)
    pnt_dists_from_median_in_z = (np.abs(xyzs_sorted - median))[:,2]

    metric_histogram = plt.hist(pnt_dists_from_median_in_z, bins='auto')
    plt.title("Distance of points to z-median of: " + str(median[2]))
    plt.xlabel("Distance")
    plt.ylabel("Number of points")
    plt.savefig(output_save_dir)
    plt.show()
    return np.sum(pnt_dists_from_median_in_z)/len(pnt_dists_from_median_in_z)
