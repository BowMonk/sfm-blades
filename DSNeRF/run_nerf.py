...
from load_llff import load_llff_data, load_colmap_depth, load_colmap_llff, load_colmap_depth_new
...
print(device, print(torch.cuda.is_available()))
...
def batchify_rays(rays_flat, chunk=1024*16, **kwargs):
...
def render(H, W, focal, chunk=1024*16, rays=None, c2w=None, ndc=True,
                  near=0., far=1.,
...
def render_path(render_poses, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        if savedir is not None:
            ...
            imageio.imwrite(os.path.join(savedir, '{:03d}_depth.png'.format(i)),np.clip(255.0 / depth.max() * (depth - depth.min()), 0, 255).astype(np.uint8))
...
def train():
    ...
    if args.dataset_type == 'colmap_llff':
        ...
        if args.colmap_depth:
            depth_gts = load_colmap_depth_new(args.datadir, factor=args.factor, bd_factor=.75)
    ...
    for i in trange(start, N_iters):
        ...
        my_temp_target = norm(target_s)
        my_temp_rgb = norm(rgb)

        # torch_pd = my_temp_rgb.view(-1, 480, 360, 3).permute(0, 3, 1, 2).to("cuda")
        # torch_gt = my_temp_target.view(-1, 480, 360, 3).permute(0, 3, 1, 2).to("cuda")
        # lpips_value = lpips_loss(torch_pd, torch_gt).detach().cpu().numpy()
        # lpips = calculate_lpips(my_temp_rgb, my_temp_target, device)
        # ssim = calculate_ssim(rgb, target_s)
        ...
        if args.i_video > 0 and i%args.i_video==0 and i > 0:
            ...
            with torch.no_grad():
                ...
                rgbs, disps = render_path(render_poses, hwf, 1024, render_kwargs_test)
        ...
        if i%args.i_testset==0 and i > 0 and len(i_test) > 0:
            ...
            with torch.no_grad():
                ...
                rgbs, disps = render_path(torch.Tensor(poses[i_test][:2]).to(device), hwf, 1024, render_kwargs_test,
                                          gt_imgs=images[i_test][:2], savedir=testsavedir)
            ...
            ### ADDED FOR METRICS CALCULATION ###
            my_temp_target = norm(images[i_test][0])
            my_temp_rgb = norm(torch.Tensor(rgbs[0]).to(device))
            torch_pd = my_temp_rgb.view(-1, 358, 479, 3).permute(0, 3, 1, 2).to("cuda")
            torch_gt = my_temp_target.view(-1, 358, 479, 3).permute(0, 3, 1, 2).to("cuda")

            lpips_value = lpips_loss(torch_pd, torch_gt).detach().cpu().numpy()
            ssim_value = ssim(rgbs[0], images[i_test][0].detach().cpu().numpy())
            ### ADDED FOR METRICS CALCULATION ###
        ...
        if i%args.i_print==0:
            ...
            # tqdm.write(f"[TRAIN] Iter: {i} Loss: {loss.item()}  PSNR: {psnr.item()} LPIPS: {lpips} SSIM: {ssim}")

            print('iter time {:.05f}'.format(dt))

            writer.add_scalar('loss', loss, global_step)
            writer.add_scalar('psnr', psnr, global_step)
            # writer.add_scalar('lpips', lpips, global_step)
            # writer.add_scalar('ssim', ssim, global_step)
            writer.add_histogram('tran', trans)
            if args.N_importance > 0:
            ...
            if i%args.i_img==0:
                ...
                with torch.no_grad():
                    rgb, disp, acc, depth, extras = render(H, W, focal, chunk=args.chunk, rays=batch_rays,
                                                           verbose=i < 10, retraw=True,
                                                           **render_kwargs_test)
                    my_temp_target = norm(target)
                    my_temp_rgb = norm(rgb)
                    torch_pd = my_temp_rgb.to("cuda")
                    torch_gt = my_temp_target.to("cuda")
                    # lpips_value = lpips_loss(torch_pd, torch_gt).detach().cpu().numpy()
                ...
                if args.colmap_depth:
                    target_so = batch[2]
                    rgb = rgb[:N_batch, :]
                    psnr = mse2psnr(img2mse(rgb, target_so))
                    writer.add_scalar('psnr_holdout', psnr, global_step)
                # writer.add_image('rgb_holdout', target.cpu().numpy()[None])
