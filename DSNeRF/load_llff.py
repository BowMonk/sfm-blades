...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True):
    ...
    print(sorted(os.listdir(os.path.join(basedir, 'images'))))
...
def render_path_spiral(c2w, up, rads, focal, zdelta, zrate, rots, N):
    ...
    # rads = np.array([1.,1.,1.,1.])
    # rads = rads - rads
    # print("bruhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh")
    # print(rads)
    hwf = c2w[:,4:5]
    
    for theta in np.linspace(0., 2. * np.pi * rots, N+1)[:-1]:
        print(c2w[:3,:4], print(c2w))
...
def load_colmap_depth_new(basedir, factor=8, bd_factor=.75):
    data_file = Path(basedir) / 'colmap_depth.npy'
    # print(data_file)
    images = read_images_text(Path(basedir) / 'images.txt')
    points = read_points3D_text(Path(basedir) / 'points3D.txt')
    ...
    for id_im in range(1, len(images) + 1):
        ...
        for i in range(len(images[int(id_im)].xys)):
